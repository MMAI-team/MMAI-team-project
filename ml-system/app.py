...
def get_predictions_transformer(data) -> "transformer":
    ...
    predictions = transformer.predict(X) # Get prediction on all of the transactions, but we need only the specific
    
    return float(predictions[0][index][1])


def get_predictions_fnn(data) -> 'fnn':
    X, _ = DataLoader(batch_size = 2048).load_submit(data=data.iloc[len(data) - 1:],
                      preprocess=True, divide=False, params_path='dataset_config.json',
                      columns_to_delete = [ 'city', 'dob', 'job',  'first', 'last','trans_date_trans_time','category','trans_num',
                                            'lat','longs','merch_lat','merch_long','unix_time','street','merchant','state','zip','gender'],
                      normalization=['amt', 'delta_time', 'distance', 'city_pop', 'age', 'hour', 'weekday'])
...
def submit():
    # Receives data from the server (transaction to process + 29 historical transactions)
